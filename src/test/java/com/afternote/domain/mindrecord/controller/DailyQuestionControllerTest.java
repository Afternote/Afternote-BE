...
import com.afternote.domain.mindrecord.service.DailyQuestionService;
...
class DailyQuestionControllerTest {
    ...
    @InjectMocks
    private DailyQuestionController dailyQuestionController;
    ...
    @BeforeEach
    void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(dailyQuestionController)
                .setCustomArgumentResolvers(new UserIdTestArgumentResolver())
                .build();
    }

    @Test
    @DisplayName("오늘의 데일리 질문 조회 API 성공")
    void getDailyQuestion_Success() throws Exception {
        given(dailyQuestionService.getTodayDailyQuestion(USER_ID)).willReturn(null);

        mockMvc.perform(get("/daily-question").requestAttr(UserIdArgumentResolver.USER_ID_ATTRIBUTE, USER_ID))
                .andExpect(status().isOk());

        verify(dailyQuestionService).getTodayDailyQuestion(USER_ID);
    }
