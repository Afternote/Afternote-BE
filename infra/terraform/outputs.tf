...
output "app_log_group_name" {
  description = "Docker awslogs group for afternote-server"
  value       = aws_cloudwatch_log_group.app.name
}

output "app_log_group_url" {
  description = "Open app logs in CloudWatch"
  value       = "https://${var.aws_region}.console.aws.amazon.com/cloudwatch/home?region=${var.aws_region}#logsV2:log-groups/log-group/${urlencode(aws_cloudwatch_log_group.app.name)}"
}
