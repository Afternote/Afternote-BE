...
| 구성 | 실행 시점 | 책임 |
| --- | --- | --- |
| `ci.yml` | `main` 대상 PR·push | dependency review, 일반 JUnit, 현재 `Dockerfile` 이미지 build를 병렬 검증하고 `CI / build`에서 집계 |
...
기본 PR 필수 검사는 이름이 고정된 `CI / build`로 설정한다. `dependency-review`, `gradle-test`, `docker-image`는 병렬 실행하며, 집계 job은 PR에서 세 job을 모두 요구한다. `main` push에서는 이벤트 특성상 실행되지 않는 dependency review만 `skipped`로 허용하고 나머지 실패·취소는 실패로 처리한다.
...
합의되지 않았거나 아직 존재하지 않는 job은 성공 처리용 `skip` 또는 빈 placeholder로 만들지 않는다. 현재 PR CI는 실제 EC2·RDS·Datadog·Sentry 전송·외부 API를 재현하지 않으며, 이를 기본 CI 통과로 검증됐다고 간주하지 않는다.
